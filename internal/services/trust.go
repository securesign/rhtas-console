...
func NewTrustService() TrustService {
	...
	db, err := sql.Open("mysql", os.Getenv("DB_DSN")) // example: user:password@tcp(localhost:3306)/tuf_trust
	if err != nil {
		log.Fatal("failed to connect to MariaDB: %v", err)
	}
	if err = db.Ping(); err != nil {
		log.Fatal("failed to ping MariaDB: %v", err)
	}
	...
	defaultRepoURL := os.Getenv("TUF_REPO_URL")
	if defaultRepoURL == "" {
		defaultRepoURL = publicGoodInstance
	}
	repo, err := s.getOrCreateUpdater(defaultRepoURL)
	if err != nil {
		log.Fatalf("Failed to initialize default TUF repository %s: %v", defaultRepoURL, err)
	}
	ctx := context.Background()
	if err := s.populateTargets(ctx, repo); err != nil {
		log.Printf("Failed to perform initial targets population for %s: %v", defaultRepoURL, err)
	}
...
func (s *trustService) GetTargetsList(ctx context.Context, tufRepoUrl string) (models.TargetsList, error) {
	...
	if targetsMeta == nil {
		return models.TargetsList{}, fmt.Errorf("targets metadata not available")
	}

	targetsBytes, err := targetsMeta.ToBytes(true)
	if err != nil {
		return models.TargetsList{}, fmt.Errorf("failed to get targets bytes: %w", err)
	}

	targetMetadataSpecs, err := extractTargetMetadataInfo(targetsBytes)
	result := models.TargetsList{}
	if err != nil {
		return models.TargetsList{}, fmt.Errorf("extracting target metadata: %w", err)
	}
	for target, info := range targetMetadataSpecs {
		target_content, err := s.GetTarget(ctx, tufRepoUrl, target)
		if err != nil {
			return models.TargetsList{}, fmt.Errorf("extracting target content: %w", err)
		}
...
func (s *trustService) GetCertificatesInfo(ctx context.Context, tufRepoUrl string) (models.CertificateInfoList, error) {
	...
	if err != nil {
		return models.CertificateInfoList{}, fmt.Errorf("extracting target metadata: %w", err)
	}
	for target, info := range targetMetadataSpecs {
...
func (s *trustService) populateTargets(ctx context.Context, repo *tufRepository) error {
	...
	for target, info := range targetMetadataSpecs {
		target_content, err := s.GetTarget(ctx, repo.remoteMetadataURL, target)
		if err != nil {
